...
void RenderFunction()
{
    ...
    static unsigned int uiFrameNumber = 0;
    uiFrameNumber++;
    ...
    case ENone:
        ...
        break;
    case EHorizontal:
        worldTransf = glm::translate(worldTransf, glm::vec3(glm::sin(uiFrameNumber/1000.f), 0.0f, 0.0f));
        break;
    case EVertical:
        worldTransf = glm::translate(worldTransf, glm::vec3(0.0f, glm::sin(uiFrameNumber / 1000.f), 0.0f));
        break;
    case ECircle:
        worldTransf = glm::translate(worldTransf, glm::vec3(glm::sin(uiFrameNumber / 1000.f), glm::cos(uiFrameNumber / 1000.f), 0.0f));
        break;
    case ERotation:
        worldTransf = glm::rotate(worldTransf, glm::radians(uiFrameNumber / 10.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        break;
    case EScale:
        worldTransf = glm::scale(worldTransf, glm::vec3(glm::abs(glm::sin(uiFrameNumber / 1000.0f)), glm::abs(glm::sin(uiFrameNumber / 1000.0f)), 0.0f));
        break;
    default:
        break;
    }
...
static void Keyboard(unsigned char Key, int x, int y)
{
    ...
    if (tolower(Key) == 'h') {
        ...
        g_movementType = EMovementType::EHorizontal;
    }
    if (tolower(Key) == 'v') {
        // vertical translacction
        g_movementType = EMovementType::EVertical;
    }
    if (tolower(Key) == 'c') {
        // vertical and horizontal translation
        g_movementType = EMovementType::ECircle;
    }
    if (tolower(Key) == 'z') {
        // rotation aroun OZ
        g_movementType = EMovementType::ERotation;
    }
    if (tolower(Key) == 's') {
        // scaling
